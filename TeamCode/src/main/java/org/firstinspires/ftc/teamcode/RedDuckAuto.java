...
public class RedDuckAuto extends LinearOpMode {
    ...
    private static final int TARGET_POSITION_TOP = -650;
    private static final int TARGET_POSITION_MID = -500;
    private static final int TARGET_POSITION_LOW = -453;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(!opModeIsActive()) {
            ...
            if (team_element_x != 0 && team_element_x < 175) {
                liftHeight = "low";
            } else if (team_element_x > 175) {
        ...
        Trajectory builder3 = drive.trajectoryBuilder(drive.getPoseEstimate())
                .forward(5, SampleMecanumDrive.getVelocityConstraint(50, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .build();
 ...
 */
    ...
    private void lift_down(){
        ...
        robot.lift.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
        runtime.reset();
        while (robot.lift.isBusy() && runtime.milliseconds() < 1000) {
            robot.lift.setPower((1));
        }
        robot.lift.setPower(0);
    }
